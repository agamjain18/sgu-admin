...
import { Save, X, Image as ImageIcon, Upload, Info, Loader2, ClipboardList, Settings, Plus, Trash2, CheckCircle2, Star, GripVertical } from 'lucide-react';
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8 animate-in zoom-in duration-500 pb-20">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold mb-2 font-serif text-slate-900">Add New Product</h1>
          <p className="text-slate-500 font-medium">Configure all product details for the global catalog.</p>
        </div>
      ...
      <form id="product-form" onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="bg-white border border-slate-200 rounded-[2rem] p-10 space-y-8 relative overflow-hidden group shadow-sm">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="md:col-span-2">
                ...
                <input 
                  ...
                  placeholder="e.g. Premium Dutch Processed Cocoa"
                  className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-6 py-4 text-base focus:outline-none focus:border-primary focus:bg-white transition-all text-slate-900 placeholder:text-slate-300"
                  required
                />
              </div>
              ...
              <div className="md:col-span-2">
                ...
                <div className="relative group/input">
                  ...
                  <input 
                    ...
                    placeholder="Type to search country (e.g. India, USA...)"
                    className="w-full bg-slate-50 border border-slate-200 rounded-2xl pl-14 pr-6 py-4 text-base focus:outline-none focus:border-primary focus:bg-white transition-all text-slate-900 placeholder:text-slate-300"
                  />
                  ...
                  {showCountrySuggestions && (
                    <div className="absolute z-50 w-full mt-2 bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-2xl max-h-64 overflow-y-auto animate-in fade-in slide-in-from-top-2 duration-200">
                      <div className="p-2 border-b border-slate-100 bg-slate-50">
                        <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest px-3">Matching Records</span>
                      </div>
                      ...
                      {(() => {
                        ...
                        if (filtered.length === 0 && countrySearch) {
                          return <div className="p-5 text-center text-slate-400 text-sm italic">No matching countries found</div>;
                        }

                        if (!countrySearch) {
                          return <div className="p-5 text-center text-slate-400 text-sm font-medium">Start typing to see hints...</div>;
                        }
                        ...
                        return filtered.map(suggestion => (
                          <button
                            ...
                            className="w-full text-left px-5 py-4 text-sm hover:bg-slate-50 text-slate-700 transition-all border-b border-slate-50 last:border-0 flex items-center justify-between group"
                          >
                            <div className="flex items-center gap-3">
                              <div className="w-2 h-2 rounded-full bg-slate-200 group-hover:bg-primary transition-colors" />
                              <span className="font-semibold">{suggestion}</span>
                            </div>
                            <Plus size={16} className="opacity-0 group-hover:opacity-100 text-primary" />
                          </button>
                        ));
                      })()}
                    </div>
              ...
              <div className="md:col-span-2">
                ...
                <input 
                  ...
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-xs mt-4 focus:outline-none focus:border-primary focus:bg-white transition-all font-medium text-slate-900"
                />
              </div>
            </div>
          </div>
          ...
          <div className="bg-white border border-slate-200 rounded-[2rem] p-10 space-y-8 shadow-sm">
            <div className="flex items-center gap-3 border-b border-slate-100 pb-6">
              ...
              <h2 className="text-2xl font-bold font-serif text-slate-900">Technical Content</h2>
            </div>
            
            <div className="space-y-8">
              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-3 flex items-center gap-2 ml-1">
                  Product Overview
                  <span className="text-[10px] bg-primary/10 text-primary px-2 py-0.5 rounded-full">Narrative</span>
                </label>
                <textarea 
                  ...
                  onChange={handleChange}
                  rows={4}
                  placeholder="Enter a comprehensive overview..."
                  className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-6 py-4 text-base focus:outline-none focus:border-primary focus:bg-white transition-all text-slate-900 placeholder:text-slate-300 resize-none"
                  required
                ></textarea>
              </div>
        ...
        <div className="space-y-6">
          <div className="bg-white border border-slate-200 rounded-[2rem] p-8 space-y-8 shadow-sm">
             <div className="flex items-center gap-3 border-b border-slate-100 pb-4">
              ...
              <h2 className="text-xl font-bold font-serif text-slate-900">Logistics</h2>
            </div>

            <div className="space-y-6">
              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-2 ml-1">Applications</label>
                <input 
                  type="text" 
                  name="applications"
              ...
              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-2 ml-1">Packaging</label>
                ...
                <input 
                  ...
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-xs focus:outline-none focus:border-purple-400 focus:bg-white transition-all text-slate-900"
                />
              </div>
              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-2 ml-1">Certifications</label>
